...
export default function AdminDashboard() {
    ...
    const isFeaturedType = (type) => {
        if (!type) return false;
        const normalized = type.toLowerCase().trim().replace(/[\s-]/g, '_');
        return ['driven_by_purpose', 'stage_behind_story', 'stage_behind_the_story', 'founders_unfiltered'].includes(normalized);
    };
    ...
    const handleDelete = async (id, permanent = false) => {
        ...
        try {
            const res = await fetch(`${apiBaseURL}/api/posts.php?id=${id}${permanent ? '&permanent=true' : ''}`, {
                method: 'DELETE',
            });
            const data = await res.json();
            if (data.status === 'success') {
    ...
    const handleDeleteSubmission = async (id) => {
        if (!window.confirm("Are you sure you want to delete this submission? This will permanently remove it from the dashboard database.")) return;
    ...
    const saveSubmissionEdits = async () => {
        try {
            const { 
                full_name, role, email, phone, company_name, company_website, company_linkedin, company_email,
                ...formDetails 
            } = editFormData;
    ...
    const filteredPosts = activeTab === 'all' || activeTab === 'trash'
        ? posts 
    ...
    const renderAsset = (key, value) => {
        ...
        return (
            <div className="submission-detail-row" key={key}>
                <div className="submission-detail-label" style={{ textTransform: 'capitalize' }}>
                    {key.replace(/_/g, ' ')}
                </div>
                <div className="submission-detail-val">
                    <div className="admin-asset-preview-list">
                        {list.map((url, i) => {
                            ...
                            return isImage ? (
                                <a href={`${apiBaseURL}${url}`} target="_blank" rel="noreferrer" key={i}>
                                    <img src={`${apiBaseURL}${url}`} alt="Asset Thumbnail" className="admin-asset-thumb" />
                                </a>
                            ) : (
                                <a href={`${apiBaseURL}${url}`} target="_blank" rel="noreferrer" className="admin-asset-link" key={i}>
                                    View File {i + 1}
                                </a>
                            );
                        })}
    ...
    const pendingCount = submissions.filter(sub => sub.status === 'pending' && !readSubmissions.includes(sub.id)).length;
        ...
        <div className="admin-layout-container">
            ...
            <div className="admin-sidebar" data-lenis-prevent>
                ...
                <div style={{ marginTop: 'auto' }}>
                    <button onClick={handleLogout} className="admin-btn admin-btn-danger" style={{ width: '100%' }}>Logout</button>
                </div>
            </div>
                ...
                {currentNav === 'posts' ? (
                    ...
                    <div className="admin-page" style={{ padding: 0 }}>
                        <div className="admin-header">
                            ...
                            <div className="admin-header-actions">
                                <Link to="/admin/editor" className="admin-btn admin-btn-primary">+ Create New Post</Link>
                            </div>
                        </div>
                                    ...
                                    <tbody>
                                        {filteredPosts.map(post => (
                                            <tr key={post.id}>
                                                ...
                                                <td className="admin-likes-count" style={{ textAlign: 'center', lineHeight: '1.4' }}>
                                                    <div style={{ fontSize: '11px', color: '#38a169', fontWeight: '600' }}>
                                                        Genuine: {Math.max(0, (post.likes_count || 0) - (post.initial_likes || 0))}
                                                    </div>
                                                    <div style={{ fontSize: '11px', color: '#dd6b20', fontWeight: '600' }}>
                                                        Default: {post.initial_likes || 0}
                                                    </div>
                                                    <div style={{ fontSize: '13px', fontWeight: 'bold', color: '#e53e3e', marginTop: '2px' }}>
                                                ...
                                                <td className="admin-date-cell" style={{ color: '#666', fontSize: '13px' }}>
                                                    {new Date(post.published_date).toLocaleDateString()}
                                                </td>
                                                <td className="admin-actions-cell" style={{ textAlign: 'right' }}>
                                                    {activeTab === 'trash' ? (
                                                        <>
                                                            <button onClick={() => handleRestore(post.id)} className="admin-btn-link" style={{ color: '#2c7a7b' }}>Restore</button>
                                                        </>
                                                    ) : (
                                                        <>
                                                            <Link to={`/admin/editor/${post.id}`} className="admin-btn-link" style={{ marginRight: '15px' }}>Edit</Link>
                                                            <button onClick={() => handleDelete(post.id)} className="admin-btn-link admin-btn-link-danger">Trash</button>
                                                        </>
                                                    )}
                                                </td>
                                            </tr>
                    ...
                    <div className="admin-page" style={{ padding: 0 }}>
                        <div className="admin-header">
                            ...
                            <p style={{ color: '#718096', fontSize: '14px', marginTop: '5px' }}>
                                A directory of submitters whose Driven By Purpose stories have been approved.
                            </p>
                        </div>
                        ...
                        <div className="admin-card">
                            <table className="admin-table">
                                ...
                                <tbody>
                                    {submissions
                                        .filter(sub => isFeaturedType(sub.type) && sub.status === 'approved')
                                        .map(sub => (
                                            <tr key={sub.id}>
                                                <td>
                                                    <div style={{ fontWeight: '600', color: '#2d3748' }}>{sub.full_name}</div>
                                                    <div style={{ fontSize: '12px', color: '#718096', marginTop: '2px' }}>{sub.role || '—'}</div>
                                                </td>
                                                <td>
                                                    <div style={{ fontWeight: '500', color: '#2d3748' }}>{sub.company_name || '—'}</div>
                                                    {sub.company_website ? (
                                                        <a 
                                                            href={sub.company_website} 
                                                            target="_blank" 
                                                            rel="noreferrer" 
                                                            style={{ fontSize: '12px', color: '#3182ce', textDecoration: 'none', display: 'inline-block', marginTop: '2px' }}
                                                        >
                                                            Visit Website ↗
                                                        </a>
                                                    ) : (
                                                        <span style={{ fontSize: '12px', color: '#a0aec0' }}>No Website</span>
                                                    )}
                                                </td>
                                                <td>
                                    ...
                                    {submissions.filter(sub => isFeaturedType(sub.type) && sub.status === 'approved').length === 0 && (
                                        <tr>
                    ...
                    <div className="admin-page" style={{ padding: 0 }}>
                        <div className="admin-header">
                            ...
                            <p style={{ color: '#718096', fontSize: '14px', marginTop: '5px' }}>
                                Review and approve form responses submitted by founders via GET FEATURED.
                            </p>
                        </div>

                        {/* Category Type Filter Tabs */}
                        <div className="admin-tabs" style={{ marginBottom: '15px', background: '#f7fafc', padding: '6px', borderRadius: '8px', border: '1px solid #edf2f7' }}>
                            <button 
                        ...
                        <div className="admin-tabs">
                            <button onClick={() => setSubFilter('all')} className={`admin-tab ${subFilter === 'all' ? 'admin-tab-active' : ''}`}>All Statuses</button>
                            <button onClick={() => setSubFilter('pending')} className={`admin-tab ${subFilter === 'pending' ? 'admin-tab-active' : ''}`}>Pending ⏳</button>
                            <button onClick={() => setSubFilter('approved')} className={`admin-tab ${subFilter === 'approved' ? 'admin-tab-active' : ''}`}>Approved ✓</button>
                            <button onClick={() => setSubFilter('rejected')} className={`admin-tab ${subFilter === 'rejected' ? 'admin-tab-active' : ''}`}>Rejected ✗</button>
                        </div>

                        {subLoading ? <p>Loading submissions...</p> : (
                            <div className="admin-card">
                                <table className="admin-table">
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody>
                                        {filteredSubmissions.map(sub => (
                                            <tr key={sub.id}>
                                                ...
                                                <td>
                                                    <span style={{ fontSize: '11px', background: '#edf2f7', padding: '4px 8px', borderRadius: '4px', textTransform: 'capitalize' }}>
                                                ...
                                                <td>
                                                    {isFeaturedType(sub.type) ? (
                                                        <span className={`admin-status-badge admin-badge-${sub.status}`}>
                                                ...
                                                <td style={{ textAlign: 'right' }}>
                                                    <button onClick={() => handleViewSub(sub)} className="admin-btn-link" style={{ marginRight: '15px' }}>View</button>
                                                    {isFeaturedType(sub.type) && (
                                                        <button onClick={() => handleOpenEdit(sub)} className="admin-btn-link" style={{ marginRight: '15px', color: '#2b6cb0' }}>Edit</button>
                                                    )}
                                                    <button onClick={() => handleDeleteSubmission(sub.id)} className="admin-btn-link admin-btn-link-danger">Delete</button>
                                                </td>
                                            </tr>
                                        ))}
            ...
            {selectedSub && !isEditMode && (
                <div className="admin-modal-overlay" onClick={() => setSelectedSub(null)} data-lenis-prevent>
                    <div className="admin-modal-content" onClick={(e) => e.stopPropagation()} data-lenis-prevent>
                        <button className="admin-modal-close" onClick={() => setSelectedSub(null)}>×</button>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                            ...
                            {isFeaturedType(selectedSub.type) && (
                                <span className={`admin-status-badge admin-badge-${selectedSub.status}`} style={{ fontSize: '14px', padding: '6px 16px' }}>
                        ...
                        {selectedSub.post_id && selectedSub.post_deleted_at && (
                            <div style={{ 
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    ...
                                    <span style={{ color: '#dd6b20', fontWeight: '600', fontSize: '14px' }}>
                                        The draft post generated from this submission is currently in the Trash.
                                    </span>
                                </div>
                                <div style={{ display: 'flex', gap: '10px' }}>
                                    <button
                                        type="button"
                                        onClick={async () => {
                                            if (window.confirm("Are you sure you want to restore the associated draft post?")) {
                                        ...
                                        style={{ backgroundColor: '#3182ce', borderColor: '#3182ce', fontSize: '13px', padding: '6px 12px' }}
                                    >
                                        Restore Post
                                    </button>
                                </div>
                        ...
                        <div className="submission-detail-row">
                            ...
                            <div className="submission-detail-val">{selectedSub.full_name}</div>
                        </div>
                        <div className="submission-detail-row">
                            <div className="submission-detail-label">Role</div>
                            <div className="submission-detail-val">{selectedSub.role || 'N/A'}</div>
                        </div>
                        <div className="submission-detail-row">
                        ...
                        <div className="submission-detail-row">
                            ...
                            <div className="submission-detail-val">{selectedSub.phone || 'N/A'}</div>
                        </div>
                        <div className="submission-detail-row">
                            <div className="submission-detail-label">Company Name</div>
                            <div className="submission-detail-val">{selectedSub.company_name || 'N/A'}</div>
                        </div>
                        <div className="submission-detail-row">
                            <div className="submission-detail-label">Company Website</div>
                            <div className="submission-detail-val">
                                {selectedSub.company_website ? (
                                    <a href={selectedSub.company_website} target="_blank" rel="noreferrer" style={{ color: '#3182ce' }}>
                                        {selectedSub.company_website}
                                    </a>
                                ) : 'N/A'}
                            </div>
                        ...
                        <div className="submission-detail-row">
                            ...
                            <div className="submission-detail-val">
                                {selectedSub.company_linkedin ? (
                                    <a href={selectedSub.company_linkedin} target="_blank" rel="noreferrer" style={{ color: '#3182ce' }}>
                                        {selectedSub.company_linkedin}
                                    </a>
                                ) : 'N/A'}
                            </div>
                        ...
                        <div className="submission-detail-row">
                            ...
                            <div className="submission-detail-val">{selectedSub.company_email || 'N/A'}</div>
                        </div>

                        {/* Submission Specific Form Content */}
                        <div className="submission-section-title" style={{ textTransform: 'capitalize' }}>
                        ...
                        {Object.keys(selectedSub.form_data || {}).map(key => {
                            ...
                            if (['company_logo', 'founder_images', 'screenshots', 'brand_images', 'additional_visuals'].includes(key)) {
                            ...
                            return (
                                <div className="submission-detail-row" key={key}>
                                    <div className="submission-detail-label" style={{ textTransform: labelMap[key] ? 'none' : 'capitalize' }}>
                                        {displayLabel}
                                    </div>
                                    <div className="submission-detail-val" style={{ whiteSpace: 'pre-wrap', lineHeight: '1.6' }}>
                                        {val || 'N/A'}
                                    </div>
                                </div>
                            );
                        ...
                        {/* Modal Action Footer */}
                        <div className="admin-form-actions" style={{ borderTop: '1px solid #edf2f7', paddingTop: '20px', marginTop: '30px' }}>
                            {isFeaturedType(selectedSub.type) && (
                                <>
                                    {selectedSub.status === 'pending' && (
                                        <>
                                            <button 
                                                onClick={() => handleSubmissionAction(selectedSub.id, 'reject')}
                                                className="admin-btn admin-btn-danger"
                                            >
                                                Reject Submission
                                            </button>
                                            <button 
                                                onClick={() => handleSubmissionAction(selectedSub.id, 'approve')}
                                                className="admin-btn admin-btn-primary"
                                            >
                                                Approve & Post Draft
                                            </button>
                                    ...
                                    {selectedSub.status === 'approved' && selectedSub.post_id && (
                                        <Link 
                                            to={`/admin/editor/${selectedSub.post_id}`}
            ...
            {selectedSub && isEditMode && (
                <div className="admin-modal-overlay" data-lenis-prevent>
                    <div className="admin-modal-content" data-lenis-prevent>
                        <button className="admin-modal-close" onClick={() => { setIsEditMode(false); setSelectedSub(null); }}>×</button>
                        
                        <h2 style={{ fontSize: '24px', fontWeight: '700', color: '#1a202c', marginBottom: '25px' }}>
                        ...
                        <form onSubmit={handleSaveEdit} className="admin-edit-form">
                            ...
                            {isDrivenByPurpose(selectedSub.type) && (
                                <>
                                    <div className="submission-section-title">Driven by Purpose Form details</div>
                                        ...
                                        <input 
                                            ...
                                            onChange={(e) => {
                                                ...
                                                setEditFormData((prev) => ({ 
                                                    ...
                                                    slug: !prev.slug || prev.slug === slugify(prev.feature_title) ? generatedSlug : prev.slug
                                                }));
                                            }}
                                            required
                                        />
                                    ...
                                    <div>
                                        ...
                                        <input 
                                            ...
                                            onChange={(e) => setEditFormData({ ...editFormData, outer_heading: e.target.value })}
                                    ...
                                    <div>
                                        ...
                                        <input 
                                            ...
                                            onChange={(e) => setEditFormData({ ...editFormData, inner_heading: e.target.value })}
                                    ...
                                    <div>
                                        ...
                                        <input 
                                            ...
                                            onChange={(e) => setEditFormData({ ...editFormData, company_tagline: e.target.value })}
                                            required
                                        />
                                    </div>

                                    <div>
                                        <label className="admin-label">Company Description (800-1000 words)</label>
                                        <textarea 
                                        ...
                                        <input 
                                            ...
                                            onChange={(e) => {
                                                ...
                                                const formattedSlug = val
                                                    ...
                                                    .replace(/\s+/g, '-')
                                                    .replace(/[^a-z0-9\-]/g, '');
                                                setEditFormData((prev) => ({ ...prev, slug: formattedSlug }));
                                            }}
                                    ...
                                    <div>
                                        <label className="admin-label">Meta Description (Title Description)</label>
                                        <textarea 
                                            className="admin-textarea" 
                                            style={{ height: '80px' }}
                                    ...
                                    <div>
                                        <label className="admin-label">Additional Notes or Links</label>
                                        <textarea 
                                            className="admin-textarea" 
                            ...
                            {(selectedSub.type === 'stage_behind_story' || selectedSub.type === 'stage_behind_the_story') && (
                                <>
                                    <div className="submission-section-title">Stage Behind the Story details</div>
                                    ...
                                    <div>
                                        ...
                                        <textarea 
                                            ...
                                            onChange={(e) => setEditFormData({ ...editFormData, one_sentence_pitch: e.target.value })}
                                            required
                                        />
                                    </div>

                                    <div>
                                        <label className="admin-label">What is one belief or experience that defines your approach?</label>
                                        <textarea 
                                            className="admin-textarea" 
                                            style={{ height: '150px' }}
                                            value={editFormData.belief_or_experience} 
                                            onChange={(e) => setEditFormData({ ...editFormData, belief_or_experience: e.target.value })}
                                            required
                                        />
                                    </div>

                                    <div>
                                        <label className="admin-label">A rarely asked story about how you started:</label>
                                        <textarea 
                                            className="admin-textarea" 
                                            style={{ height: '150px' }}
                                            value={editFormData.rarely_asked_story} 
                                            onChange={(e) => setEditFormData({ ...editFormData, rarely_asked_story: e.target.value })}
                                            required
                                        />
                                    </div>

                                    <div>
                                        <label className="admin-label">Why do you belong in this space?</label>
                                        <textarea 
                                            className="admin-textarea" 
                                            style={{ height: '150px' }}
                                            value={editFormData.why_belongs} 
                                            onChange={(e) => setEditFormData({ ...editFormData, why_belongs: e.target.value })}
                                            required
                                        />
                                    </div>
                                </>
                            ...
                            {selectedSub.type === 'founders_unfiltered' && (
                                <>
                                    <div className="submission-section-title">Founders Unfiltered questions</div>
                                    ...
                                    <div>
                                        <label className="admin-label">1. Introduce yourself in one line</label>
                                        <input 
                                            type="text" 
                                            className="admin-input" 
                                            value={editFormData.q1} 
                                            onChange={(e) => setEditFormData({ ...editFormData, q1: e.target.value })}
                                            required
                                        />
                                    </div>

                                    <div>
                                        <label className="admin-label">2. What are you building?</label>
                                        <textarea 
                                            className="admin-textarea" 
                                            style={{ height: '100px' }}
                                            value={editFormData.q2} 
                                            onChange={(e) => setEditFormData({ ...editFormData, q2: e.target.value })}
                                            required
                                        />
                                    </div>

                                    <div>
                                        <label className="admin-label">3. Why this problem? Why now?</label>
                                        <textarea 
                                            className="admin-textarea" 
                                            style={{ height: '100px' }}
                                            value={editFormData.q3} 
                                            onChange={(e) => setEditFormData({ ...editFormData, q3: e.target.value })}
                                            required
                                        />
                                    </div>

                                    <div>
                                        <label className="admin-label">4. What does a normal day in your life actually look like?</label>
                                        <textarea 
                                            className="admin-textarea" 
                                            style={{ height: '100px' }}
                                            value={editFormData.q4} 
                                            onChange={(e) => setEditFormData({ ...editFormData, q4: e.target.value })}
                                            required
                                        />
                                    </div>

                                    <div>
                                        <label className="admin-label">5. What's one thing startup culture gets completely wrong?</label>
                                        <textarea 
                                            className="admin-textarea" 
                                            style={{ height: '100px' }}
                                            value={editFormData.q5} 
                                            onChange={(e) => setEditFormData({ ...editFormData, q5: e.target.value })}
                                            required
                                        />
                                    </div>

                                    <div>
                                        <label className="admin-label">6. What has building taught you about yourself?</label>
                                        <textarea 
                                            className="admin-textarea" 
                                            style={{ height: '100px' }}
                                            value={editFormData.q6} 
                                            onChange={(e) => setEditFormData({ ...editFormData, q6: e.target.value })}
                                            required
                                        />
                                    </div>

                                    <div>
                                        <label className="admin-label">7. What's your current biggest challenge?</label>
                                        <textarea 
                                            className="admin-textarea" 
                                            style={{ height: '100px' }}
                                            value={editFormData.q7} 
                                            onChange={(e) => setEditFormData({ ...editFormData, q7: e.target.value })}
                                            required
                                        />
                                    </div>

                                    <div>
                                        <label className="admin-label">8. What's one opinion you have that most founders may disagree with?</label>
                                        <textarea 
                                            className="admin-textarea" 
                                            style={{ height: '100px' }}
                                            value={editFormData.q8} 
                                            onChange={(e) => setEditFormData({ ...editFormData, q8: e.target.value })}
                                            required
                                        />
                                    </div>

                                    <div>
                                        <label className="admin-label">9. What advice would you give someone starting today?</label>
                                        <textarea 
                                            className="admin-textarea" 
                                            style={{ height: '100px' }}
                                            value={editFormData.q9} 
                                            onChange={(e) => setEditFormData({ ...editFormData, q9: e.target.value })}
                                            required
                                        />
                                    </div>

                                    <div>
                                        <label className="admin-label">10. Complete this sentence: "People think entrepreneurship is ________, but actually it's ________."</label>
                                        <input 
                                            type="text" 
                                            className="admin-input" 
                                            value={editFormData.q10} 
                                            onChange={(e) => setEditFormData({ ...editFormData, q10: e.target.value })}
                                            required
                                        />
                                    </div>

                                    <div>
                                        <label className="admin-label">11. Anything unfiltered you'd like to leave readers with?</label>
                                        <textarea 
                                            className="admin-textarea" 
                                            style={{ height: '120px' }}
                                            value={editFormData.q11} 
                                            onChange={(e) => setEditFormData({ ...editFormData, q11: e.target.value })}
                                            required
                                        />
                                    </div>
                                </>
                            ...
                            <div className="admin-form-actions" style={{ borderBottom: '1px solid #edf2f7', paddingBottom: '20px', marginBottom: '20px' }}>
                                <button 
                                    type="button" 
                                    onClick={() => { setIsEditMode(false); setSelectedSub(null); }} 
                                    className="admin-btn admin-btn-secondary"
                                >
                                    Cancel
                                </button>
                             ...
                             {isFeaturedType(selectedSub.type) && (
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <div style={{ color: '#718096', fontSize: '13px' }}>
                                        Submission Status: <strong style={{ textTransform: 'uppercase', color: selectedSub.status === 'pending' ? '#dd6b20' : '#2b6cb0' }}>{selectedSub.status}</strong>
                                    </div>
                                    <div style={{ display: 'flex', gap: '10px' }}>
                                        {selectedSub.status === 'pending' && (
                                            <>
