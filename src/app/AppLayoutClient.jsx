...
export default function AppLayoutClient({ children }) {
  ...
  React.useEffect(() => {
    const fetchDynamicPosts = async () => {
      try {
        ...
        // Ensure response is actually JSON and not HTML error page
  ...
  const isValidRoute = () => {
    ...
    if (cleanPath[0] === "blogs" && cleanPath[1]) {
      const isStatic = blogMetadata.some(b => b.slug === cleanPath[1]);
      if (isStatic) return true;
      return dynamicPosts.some(p => p.categorySlug === "blogs" && p.slug === cleanPath[1]);
    }

    // Check success-wire routes: /success-wire/:slug
    if (cleanPath[0] === "success-wire" && cleanPath[1]) {
      const isStatic = successLensMetadata.some(l => l.slug === cleanPath[1]);
      if (isStatic) return true;
      return dynamicPosts.some(p => p.categorySlug === "success-wire" && p.slug === cleanPath[1]);
    }

    // Check events routes: /events/:slug
    if (cleanPath[0] === "events" && cleanPath[1]) {
      const isStatic = eventMetadata.some(e => e.slug === cleanPath[1]);
      if (isStatic) return true;
      return dynamicPosts.some(p => p.categorySlug === "events" && p.slug === cleanPath[1]);
    }

    // Check pod routes: /:categorySlug/:slug (e.g., /stage-behind-the-story/nexgen-software)
    if (cleanPath[0] && cleanPath[1]) {
      const isStatic = podMetadata.some(p => p.categorySlug === cleanPath[0] && p.slug === cleanPath[1]);
      if (isStatic) return true;
      return dynamicPosts.some(p => p.categorySlug === cleanPath[0] && p.slug === cleanPath[1]);
    }
  ...
  const isExcludedPage =
    pathname.startsWith("/driven-by-purpose/") ||
    pathname.startsWith("/founders-unfiltered/") ||
    pathname.startsWith("/stage-behind-the-story/") ||
    pathname.startsWith("/success-wire/") ||
    pathname.startsWith("/events/") ||
    pathname === "/about" ||
    pathname === "/get-featured" ||
