...
// Content injection variables — populated when a DB post is found
$ssrHeading = '';
$ssrContent = '';
...
$staticPages = [
    '/success-wire' => [
        'title' => 'Success Wire - SuccessWikis',
        'description' => 'Stay updated with the latest startup ecosystem news, highlights, insights and releases on SuccessWikis.'
    ],
    '/blogs' => [
        'title' => 'Blogs - SuccessWikis',
        'description' => 'Read insightful blogs and articles on startup growth, leadership, product engineering, design aesthetics, and the ecosystem on SuccessWikis.'
    ],
    '/about' => [
...
// 1. Blogs: /blogs/:slug (only when there's an actual sub-slug, not just /blogs)
if (preg_match('~^blogs/(.+)~i', $cleanPath, $m_blogs)) {
    ...
    if ($db_connected) {
        try {
            global $pdo;
            $stmt = $pdo->prepare("SELECT title, meta_description, image_url, slug, content, inner_heading FROM posts WHERE slug = ? AND post_type = 'blog' AND deleted_at IS NULL LIMIT 1");
            $stmt->execute([$blogId]);
            $dbItem = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($dbItem) {
                $title = $dbItem['title'] . " | SuccessWikis";
                ...
                $ssrHeading = $dbItem['inner_heading'] ?: $dbItem['title'];
                $ssrContent = $dbItem['content'] ?: '';
                $found = true;
...
// 3. Success Wire: /success-wire/:slug (only when there's an actual sub-slug)
elseif (preg_match('~^success-wire/(.+)~i', $cleanPath, $m_sw)) {
    $parts = explode('/', $cleanPath);
    $lensSlug = end($parts);
    ...
    if ($db_connected) {
        try {
            global $pdo;
            $stmt = $pdo->prepare("SELECT title, meta_description, image_url, slug, content, inner_heading FROM posts WHERE slug = ? AND post_type = 'success_lens' AND deleted_at IS NULL LIMIT 1");
...
// 4. Events: /events/:slug (only when there's an actual sub-slug)
elseif (preg_match('~^events/(.+)~i', $cleanPath, $m_ev)) {
    $parts = explode('/', $cleanPath);
    $eventSlug = end($parts);
    ...
    if ($db_connected) {
        try {
            global $pdo;
            $stmt = $pdo->prepare("SELECT title, meta_description, image_url, slug, content, inner_heading FROM posts WHERE slug = ? AND post_type = 'event' AND deleted_at IS NULL LIMIT 1");
...
// 5. Pod: /:categorySlug/:slug (only for known pod categories with a sub-slug)
elseif (preg_match('~^(driven-by-purpose|founders-unfiltered|stage-behind-the-story)/(.+)~i', $cleanPath)) {
    $parts = explode('/', $cleanPath);
    $query_err = null;
    ...
    if (count($parts) === 2) {
        ...
        if ($db_connected) {
            try {
                global $pdo;
                $stmt = $pdo->prepare("SELECT title, meta_description, image_url, slug, post_type, content, inner_heading FROM posts WHERE slug = ? AND post_type = ? AND deleted_at IS NULL LIMIT 1");
...
// 1. Determine if a matching statically pre-rendered Next.js HTML page exists
$cleanPath = trim($path, '/');
if ($cleanPath !== '') {
    $matchingHtmlFile = $cleanPath . '.html';
    if (file_exists($matchingHtmlFile)) {
        $sourceFile = $matchingHtmlFile;
    } else {
        // For dynamic admin posts with no pre-generated HTML, serve a known static shell
        // from the same category. The client components (PodSlugClient, BlogSlugClient, etc.)
        // use useParams() to read the actual URL slug and fetch the correct post from the API.
        $pathParts = explode('/', $cleanPath);
        if (count($pathParts) >= 2) {
            $categoryShells = [
                'blogs'                  => 'blogs/dont-need-figured-out.html',
                'stage-behind-the-story' => 'stage-behind-the-story/singara-mohan-kaalamega-karigindhi.html',
                'founders-unfiltered'    => 'founders-unfiltered/raghu-boddu.html',
                'driven-by-purpose'      => 'driven-by-purpose/raghav-foundation.html',
                'success-wire'           => 'success-wire/shark-tank-hoora-success.html',
                'events'                 => 'events/episode-1.html',
            ];
            $cat = $pathParts[0];
            if (isset($categoryShells[$cat]) && file_exists($categoryShells[$cat])) {
                $sourceFile = $categoryShells[$cat];
            }
        }
    }
}

// Fallback if the determined source file does not exist
if (!file_exists($sourceFile)) {
    $sourceFile = 'template.html';
}

if (!file_exists($sourceFile) && file_exists('index.html')) {
...
// Inject post content into HTML body so crawlers (Claude, Google, bots) can read it
// without executing JavaScript. Visually hidden from users; React renders on top.
if (!empty($ssrContent)) {
    $safeContent = strip_tags($ssrContent, '<p><h2><h3><h4><h5><ul><ol><li><a><strong><em><b><i><br><blockquote>');
    $ssrBlock  = "\n<!-- SSR Article Content for Crawlers -->\n";
    $ssrBlock .= '<article id="sw-ssr" style="position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0" aria-hidden="true">';
    $ssrBlock .= '<h1>' . htmlspecialchars($ssrHeading, ENT_QUOTES, 'UTF-8') . '</h1>';
    $ssrBlock .= '<p>' . htmlspecialchars($description, ENT_QUOTES, 'UTF-8') . '</p>';
    $ssrBlock .= $safeContent;
    $ssrBlock .= '</article>';
    $html = str_replace('</body>', $ssrBlock . "\n</body>", $html);
}
